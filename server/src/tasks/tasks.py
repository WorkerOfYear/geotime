...
#             return None
#
#     def check_banch_msg(self, name_queue: str, banch_size: int = 24) -> list[dict] | None:
#         self.start_connection_queue(name_queue)
#         queue_messages = []
...
def test_calibration(self, calibration: dict):
    ...
    while retries < max_retries:
        try:
            ...
            # ret, first_frame = cap.read()
            # first_frame = cv2.imread('img.png')
            # cv2.imwrite(img_name, first_frame)
            # min_flow_magnitude = 2.0

            # TODO: сделать боевую логику для колибровки
        ...
        try:
            ...
            while True:
                ...
                if status['status']:
                    content = {
                        job_id: {
                            "total_shlam_square": 41234,
                            "total_shlam_volume": 12344,
                            "average_speed": 1234,
                        },
                        "job_id": job_id
                    }
